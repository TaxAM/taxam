...
for pool in ['a', 'b', 'c', 'd', 'e']:
    ...
    for tx_level in range(tx_level_roof):
        for a_tie in range(a_tie_roof):
            row += f'python "/tmp/taxam/first_model/taxam/execTaxam.py" -fp "/tmp/taxam/first_model/tests/tmp/inputs/feb-03/pool_{pool}/espec_folder" -tl {tx_level + 1} -fu {a_tie + 1} -op "{pool}_output_level{tx_level}_tie-{tx_level_names[a_tie]}"'
